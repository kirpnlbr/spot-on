...
function ParkingGridPage({ lot, onBack }) {
  ...
  const levels = [1, 2, 3]; // Adjust according to your number of levels

  useEffect(() => {
    if (selectedLevel) {
      console.log(`Fetching parking grid for Level: ${selectedLevel}`); // Debugging
      getParkingGrid(lot.name, selectedLevel)
        .then(response => {
          ...
          if (response.data.spots) {
            setParkingGrid(response.data.spots); // Update the parking grid state
          } else {
